using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Jugador1 : MonoBehaviour {

   
    public GameObject jugador2;
    public GameObject cubo1;
    public GameObject cubo2;
    public GameObject cubo3;
    public GameObject popUp;
    public GameObject Bomba;
    float x1;
    float y1;
    float z1;
    float x2;
    float y2;
    float z2;
    float xcubo1;
    float zcubo1;
    float xcubo2;
    float zcubo2;
    float xcubo3;
    float zcubo3;
    bool flag1;
    bool flag2;
    Vector3 pos1;
    Vector3 pos2;
    Vector3 futuro1;
    Vector3 futuro2;
    public float moveSpeed;
    int upperlimitX = 5;
    int lowerlimitX = -5;
    int upperlimitZ = 5;
    int lowerlimitZ = -5;
    Vector3[] prohibidos = new Vector3[21];
    Vector3[] cubitos = new Vector3[3];
    Vector3 cubito1;
    Vector3 cubito2;
    Vector3 cubito3;
    public int contador1;
    public int contador2;
  

	void Start () {

       contador1 = 2;
    contador2 = 2;

        popUp.SetActive(false);


        //posiciones random de jugadores
        x1 = Random.Range(-4, 4);
        y1 = 1;
        z1 = -4;
        pos1 = new Vector3(x1, y1, z1);
        transform.position = pos1;
        flag1 = true;

        z2 = Random.Range(-3, 4);
        y2 = 1;
        x2 = 0;
        pos2 = new Vector3(x2, y2, z2);
        jugador2.transform.position = pos2;

        //posiciones cubos
        xcubo1= Random.Range(-2,-4);
        zcubo1=Random.Range(2, 4);
        cubito1 = new Vector3(xcubo1, y2, zcubo1);
        cubo1.transform.position = cubito1;

        xcubo2 = Random.Range(0, 2);
        zcubo2 = Random.Range(1, 4);
        cubito2 = new Vector3(xcubo2, y2, zcubo2);
        cubo2.transform.position = cubito2;

        xcubo3 = Random.Range(2, 4);
        zcubo3 = Random.Range(-1, -4);
        cubito3 = new Vector3(xcubo3, y2, zcubo3);
        cubo3.transform.position = cubito3;

        cubitos[0] = cubito1;
        cubitos[1] = cubito2;
        cubitos[2] = cubito3;

        //array de paredes 
        prohibidos[0] = new Vector3(-1, 1, -1);
        prohibidos[1] = new Vector3(-1, 1, 0);
        prohibidos[2] = new Vector3(-1, 1, 1);
        prohibidos[3] = new Vector3(-1, 1, 2);
        prohibidos[4] = new Vector3(-1, 1, 3);
        prohibidos[5] = new Vector3(-1, 1, 4);

        prohibidos[6] = new Vector3(1, 1, 0);
        prohibidos[7] = new Vector3(1, 1, -1);
        prohibidos[8] = new Vector3(1, 1, -2);
        prohibidos[9] = new Vector3(1, 1, -3);

        prohibidos[10] = new Vector3(3, 1, 0);
        prohibidos[11] = new Vector3(3, 1, 1);
        prohibidos[12] = new Vector3(3, 1, 2);
        prohibidos[13] = new Vector3(3, 1, 3);

        prohibidos[14] = new Vector3(-4, 1, -3);
        prohibidos[15] = new Vector3(-3, 1, -3);
        prohibidos[16] = new Vector3(-2, 1, -3);
        prohibidos[17] = new Vector3(-1, 1, -3);

        prohibidos[18] = new Vector3(-3, 1, 0);
        prohibidos[19] = new Vector3(-3, 1, -1);
        prohibidos[20] = new Vector3(-3, 1, -2);
	}
	
	void Update () {
        Update1();
        Update2();

            }



    // movimiento jugador 1
    void Update1()
    {
        flag1 = true;
            if (Input.GetKeyUp(KeyCode.UpArrow))
        {
            if(transform.position.z + 1 < upperlimitZ )
            {
           //  transform.Translate(Vector3.forward * moveSpeed * Time.deltaTime);
                // rb.MovePosition(Vector3.forward);
               // jugador2.transform.Translate(Vector3.forward);
                futuro1 = new Vector3(transform.position.x , transform.position.y, transform.position.z+1);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro1 == prohibidos[i])
                    {
                        flag1 = false;
                    }

                }

                for (int j = 0; j <= 2; j++)
                {
                    if (futuro1 == cubitos[j])
                    {
                        flag1 = false;
                    }

                }



                if (flag1 == true)
                {
                    transform.position += new Vector3(0, 0, 1);

                }
            }
        }
            if (Input.GetKeyUp(KeyCode.DownArrow))
  
            {
            if (transform.position.z - 1 > lowerlimitZ)
            {
               // transform.Translate(Vector3.back * moveSpeed * Time.deltaTime);
                futuro1 = new Vector3(transform.position.x, transform.position.y, transform.position.z-1);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro1 == prohibidos[i])
                    {
                        flag1 = false;
                    }

                }
                for (int j = 0; j <= 2; j++)
                {
                    if (futuro1 == cubitos[j])
                    {
                        flag1 = false;
                    }

                }



                if (flag1 == true)
                {
                    transform.position += new Vector3(0, 0, -1);

                }
            }
    }

            if (Input.GetKeyUp(KeyCode.LeftArrow))
  
            {
            if (transform.position.x - 1 > lowerlimitX)
            {
               // transform.Translate(Vector3.left * moveSpeed * Time.deltaTime);

                futuro1 = new Vector3(transform.position.x - 1, transform.position.y, transform.position.z);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro1 == prohibidos[i])
                    {
                        flag1 = false;
                    }

                }
                for (int j = 0; j <= 2; j++)
                {
                    if (futuro1 == cubitos[j])
                    {
                        flag1 = false;
                    }

                }





                if (flag1 == true)
                {
                    transform.position += new Vector3(-1, 0, 0);

                }
            }
}


        if (Input.GetKeyUp(KeyCode.RightArrow))
            {
             
           
                if (transform.position.x + 1 < upperlimitX)
            {

                futuro1 = new Vector3(transform.position.x + 1, transform.position.y, transform.position.z);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro1 == prohibidos[i])
                    {
                        flag1 = false;
                    }

                }
                for (int j = 0; j <= 2; j++)
                {
                    if (futuro1 == cubitos[j])
                    {
                        flag1 = false;
                    }

                }




                if (flag1 == true)
                {
                    transform.position += new Vector3(1, 0, 0);

                }
            }
        }
          
       if (Input.GetKeyUp("space"))
        {
            
            {
            GameObject Temporary_Bomba;
            Temporary_Bomba = Instantiate(Bomba, transform.position,transform.rotation) as GameObject;

            if ((Mathf.Abs(cubito1.x - Temporary_Bomba.transform.position.x) < 3) && (Mathf.Abs(cubito1.z - Temporary_Bomba.transform.position.z) < 3))
            {
                Invoke("Explosion1", 3.0f);
            }

            if ((Mathf.Abs(cubito2.x - Temporary_Bomba.transform.position.x) < 3) && (Mathf.Abs(cubito2.z - Temporary_Bomba.transform.position.z) < 3))
            {
                Invoke("Explosion2", 3.0f);
            }

            if ((Mathf.Abs(cubito3.x - Temporary_Bomba.transform.position.x) < 3) && (Mathf.Abs(cubito3.z - Temporary_Bomba.transform.position.z) < 3))
            {
                Invoke("Explosion3", 3.0f);
            }


            float a = Temporary_Bomba.transform.position.x;
            float b = Temporary_Bomba.transform.position.z;
            Destroy(Temporary_Bomba, 3.0f);
            StartCoroutine(Explosion4(a,b,3));
            StartCoroutine(Explosion5(a, b, 3));
            
        }

       

      


    }

    }


   //movimiento jugador 2
    void Update2()
    {
     
        flag2 = true;
        if (Input.GetKeyUp(KeyCode.W))
        {
            if (jugador2.transform.position.z + 1 < upperlimitZ)
            {
                futuro2 = new Vector3(jugador2.transform.position.x, jugador2.transform.position.y, jugador2.transform.position.z + 1);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro2 == prohibidos[i])
                    {
                        flag2 = false;
                    }

                }

                for (int j = 0; j <= 2; j++)
                {
                    if (futuro2 == cubitos[j])
                    {
                        flag2 = false;
                    }

                }



                if (flag2 == true)
                {
                    jugador2.transform.position += new Vector3(0, 0, 1);
                  

                }
            }
        }
            
           

        if (Input.GetKeyUp(KeyCode.S))

            {
            if (jugador2.transform.position.z - 1 > lowerlimitZ)
            {
                
                futuro2 = new Vector3(jugador2.transform.position.x, jugador2.transform.position.y, jugador2.transform.position.z - 1);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro2 == prohibidos[i])
                    {
                        flag2 = false;
                    }

                }
                for (int j = 0; j <= 2; j++)
                {
                    if (futuro2 == cubitos[j])
                    {
                        flag2 = false;
                    }

                }



                if (flag2 == true)
                {
                    jugador2.transform.position += new Vector3(0, 0, -1);

                }
            }
        }
         
        if (Input.GetKeyUp(KeyCode.A))

            {
            if (jugador2.transform.position.x - 1 > lowerlimitX)
            {
                

                futuro2 = new Vector3(jugador2.transform.position.x - 1, jugador2.transform.position.y, jugador2.transform.position.z);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro2 == prohibidos[i])
                    {
                        flag2 = false;
                    }

                }
                for (int j = 0; j <= 2; j++)
                {
                    if (futuro2 == cubitos[j])
                    {
                        flag2 = false;
                    }

                }





                if (flag2 == true)
                {
                    jugador2.transform.position += new Vector3(-1, 0, 0);

                }
            }
        }

        if (Input.GetKeyUp(KeyCode.D))
        {
             
           
                if (jugador2.transform.position.x + 1 < upperlimitX)
            {

                futuro2 = new Vector3(jugador2.transform.position.x + 1, jugador2.transform.position.y, jugador2.transform.position.z);
                for (int i = 0; i <= 20; i++)
                {
                    if (futuro2 == prohibidos[i])
                    {
                        flag2 = false;
                    }

                }
                for (int j = 0; j <= 2; j++)
                {
                    if (futuro2 == cubitos[j])
                    {
                        flag2 = false;
                    }

                }




                if (flag2 == true)
                {
                    jugador2.transform.position += new Vector3(1, 0, 0);

                }
            }
        }
          
        if (Input.GetKeyUp(KeyCode.Q))
        {

            GameObject Temporary_Bomba;
            Temporary_Bomba = Instantiate(Bomba, jugador2.transform.position, transform.rotation) as GameObject;

            if ((Mathf.Abs(cubito1.x - Temporary_Bomba.transform.position.x) < 3) && (Mathf.Abs(cubito1.z - Temporary_Bomba.transform.position.z) < 3))
            {
                Invoke("Explosion1", 3.0f);
            }

            if ((Mathf.Abs(cubito2.x - Temporary_Bomba.transform.position.x) < 3) && (Mathf.Abs(cubito2.z - Temporary_Bomba.transform.position.z) < 3))
            {
                Invoke("Explosion2", 3.0f);
            }

            if ((Mathf.Abs(cubito3.x - Temporary_Bomba.transform.position.x) < 3) && (Mathf.Abs(cubito3.z - Temporary_Bomba.transform.position.z) < 3))
            {
                Invoke("Explosion3", 3.0f);
            }


            float a = Temporary_Bomba.transform.position.x;
            float b = Temporary_Bomba.transform.position.z;
            Destroy(Temporary_Bomba, 3.0f);
            StartCoroutine(Explosion4(a, b, 3));
            StartCoroutine(Explosion5(a, b, 3));

        }


    }


  // explosiones cubos
    void Explosion1()
    {
        cubo1.SetActive(false);
        cubito1 = new Vector3(100, 100, 100);
    }

    void Explosion2()
    {
        cubo2.SetActive(false);
        cubito2 = new Vector3(100, 100, 100);
    }

    void Explosion3()
    {
        cubo3.SetActive(false);
        cubito3= new Vector3(100, 100, 100);
    }

    //explosion jugador 1
    IEnumerator Explosion4(float a, float b, float delay)
    {

        yield return new WaitForSeconds(delay);
     
        if ((Mathf.Abs(gameObject.transform.position.x- a) < 3) && (Mathf.Abs(gameObject.transform.position.z - b) < 3))
        {
            gameObject.SetActive(false);

          
            contador1 -= 1;

        ScoreBehaviour.AddScore();

        x1 = Random.Range(-4, 4);
            pos1 = new Vector3(x1, y1, z1);
            transform.position = pos1;
            flag1 = true;

           
           
            Result.AddScore1();
            if (contador1 == 0)
        {
            popUp.SetActive(true);

        }
       
       
            gameObject.SetActive(true);
           

           
           
        }

        }
     


    //explosion jugador 2

    IEnumerator Explosion5(float a, float b, float delay)
    {
      

        yield return new WaitForSeconds(delay);

        if ((Mathf.Abs(jugador2.transform.position.x - a) < 3) && (Mathf.Abs(jugador2.transform.position.z - b) < 3))
        {

            jugador2.SetActive(false);
            contador2 -= 1;
            ScoreBehaviour2.AddScore();

            z2 = Random.Range(-3, 4);
            pos2 = new Vector3(x2, y2, z2);
            jugador2.transform.position = pos2;

           
           
         

            Result.AddScore2();

            if (contador2 == 0)
            {
                popUp.SetActive(true);


            }
            jugador2.SetActive(true);

        }

      

    }
  


}
  

 

    



    
           
